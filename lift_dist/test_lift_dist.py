...
def test_ellipse():
    print("\nTesting ellipse -->")
    LD = LiftDistVb(1, Na=5, Ny=101, cd0_model='flat_plate',cl_model='flat_plate')
    b = 5
    V = 1
    Vb2 = (V*b)**2
    ...
    assert np.isclose(_AR, LD._AR(c_2b)), f"{_AR} is different from computed 1/AR {LD._AR(c/2/b)}"
    ...
    LD.update_bounds({"lb":{"c_2b": 0.1}, "ub":{"c_2b":0.3}})
    x0 = LD.initial_guess() 
...
def test_GP():
    ...
    LD.update_bounds({"lb":{"c_2b": 0.1}, "ub":{}})
    x, prob = LD.optimize(x0)
    assert prob.status == "optimal", ("Problem did not converge")
    Vb2, c_2b, A = LD.get_vars(x)
    assert np.allclose(c_2b, 0.3), f"c_2b {c_2b} not equal to lower bound 0.3"
