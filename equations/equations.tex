...
\begin{itemize}
	...
	% \item[]  a matrix of size $N_{\theta}, N_A$ such that: $ \forall k,n \quad {S'}_{k,n} = n\sin(n\theta_k)/\sin(\theta_k)$ 
...
% 		\right) \label{obj2} \\
% 	& \text{subject to}
% 	& & SA =  {C_L}^{2D} \left( \alpha_k + \alpha_{i_k} , Re(\theta_k)\right)  \label{llcon1}\\
